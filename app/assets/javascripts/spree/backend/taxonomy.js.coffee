...
Handlebars.registerHelper 'isItem', ->
  @is_item
...
draw_tree = (taxonomy, siblings, isSubCategory) ->
  taxons_template = HandlebarsTemplates["taxons/tree"]
  taxonomy_tree = $('#taxonomy_tree')
  taxonomy_tree
    .html( taxons_template({
    taxons: [taxonomy.root],
    siblings: siblings,
    ...
    .sortable
      ...
      tolerance: 'pointer'
      cursorAt: { left: 5 }
  taxonomy_tree.find(".siblings-opt-"+taxonomy.id).attr("selected", "selected")
  taxonomy_tree.find(".siblings-taxonomy, .siblings-taxons").select2()

redraw_tree = ->
  get_taxonomy().done(get_siblings)

get_siblings = (taxonomy) ->
  Spree.ajax
    url: "#{Spree.routes.taxonomy_sibling_path}?set=nested"
    data: { id: taxonomy.id }
    success: (siblings) ->
      draw_tree(taxonomy, siblings, Spree.routes.isSubCategory )
