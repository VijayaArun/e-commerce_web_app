      ...
      class Order
        def self.import(user, params)
          ActiveRecord::Base.transaction do
            ...
            create_params = params.slice :currency,:user_id
            ...
            create_order_details_from_params(params.delete(:order_details_attributes), order)
            ...
            #Call finalize to reflect quantity when ordering product from an api.
            order.finalize!
            order.reload
          end
        end

        def self.create_order_details_from_params(order_details, order)
          return [] unless order_details
            order_detail = order.build_order_detail(
              order_id:  order.id,
              transport_info: order_details[:transport_info],
              agent_info:  order_details[:agent_info]
            )
            order_detail.save!
        end
